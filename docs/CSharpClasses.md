...
| InvokeComCallback| Invokes a VBScript method\. The parameter <tt>callbackRef</tt> is an object reference to a VBScript member returned by the VBScript Function GetRef\.| | | Method| ComEvent| VBScripting |
...
| ExpandedResolvedInitialDirectory| Gets the initial directory with relative path resolved and environment variables expanded\. Improves testability\.| | | Property| FileChooser| VBScripting |
| Filter| Gets or sets the selectable file types\. Examples: <pre style="overflow\-x: auto;"> fc\.Filter = "All files \(\*\.\*\)\|\*\.\*" // the default <br/> fc\.Filter = "Text files \(\*\.txt\)\|\*\.txt\|All files \(\*\.\*\)\|\*\.\*" <br/> fc\.Filter = "Image Files\(\*\.BMP;\*\.JPG;\*\.GIF\)\|\*\.BMP;\*\.JPG;\*\.GIF\|All files \(\*\.\*\)\|\*\.\*" </pre>| | | Property| FileChooser| VBScripting |
...
| Extract| Extracts an icon from the specified \.dll or \.exe file\. Other parameters: <tt>number</tt> is an integer that specifies the icon's index within the resource\. <tt>largeIcon</tt> is a boolean that specifies whether the icon should be a large icon or small icon\.| an icon| file, number, largeIcon| Method| IconExtractor| VBScripting |
